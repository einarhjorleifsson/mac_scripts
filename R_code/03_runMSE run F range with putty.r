...
scen          <- c("LTMP")              # 
opt           <- 3                      # for multiple scenario combinations a counter
...
for (iYr in an(projPeriod)){
  ...
  if (LastRecOp == "geom")  for (i in 1:iTer) stock.n(stocks)[1,ac(TaY),,,,iTer] <-     exp(mean(log(stock.n(stocks)[1,ac(1990:(TaY-1)),,,,iTer])))
  ...
  #- Project 
  res                       <- projectMac(stocks[,1:idxyrmin1],survivors,tmp_fishery,iYr,TAC,mpPoints$scen,NULL,histMaxYr,mpPoints,mpOptions)
  TAC[,ac(FcY)]             <- res[["TAC"]]; HCRTAC[,ac(FcY)]          <- res[["HCRTAC"]]
