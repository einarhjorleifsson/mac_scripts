...
if(substr(R.Version()$os,1,3)== "lin"){
  path        <- sub("W:/","/media/w/",path)
  inPath      <- sub("W:/","/media/w/",inPath)
  codePath    <- sub("W:/","/media/w/",codePath)
  outPath     <- sub("W:/","/media/w/",outPath)
}
...
scen          <- c("LTMP")              # 
opt           <- 5                      # for multiple scenario combinations a counter
