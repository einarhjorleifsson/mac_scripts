...
save(Mac.sam,file=paste(assess.name,".RData",sep=""))

library(FLSAM)
load(paste(assess.name,".RData",sep=""))                 # load the FLSAM object with tha assessment
...
Mac.proj<-proj1stY(Mac.proj,2013,ImY.catch)
...
options.l <- list( # declared 2014 TACs
                  "declared 2014 TACs"=
                    projectControl(data.frame(year=c(AdY,CtY),
                                          quantity="catch",
                                          val=c(1326681,1326681))),
                  #Zero catch
                  "Catch(2014) = Zero"=
                    projectControl(data.frame(year=c(AdY,CtY),
                                          quantity="catch",
