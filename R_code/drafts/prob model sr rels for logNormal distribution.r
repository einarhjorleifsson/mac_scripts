...
pathFig <- paste(outPath,"Plots input/",sep="")
...
# Rec is the log(Rec/ 1000), SSB is SSB/1000  .
...
sllq=0
for (i in 1:SRpair) {
  mu=log(HSa[i1]*HSb[i1]*(SSB[i]>=HSb[i1])+HSa[i1]*SSB[i]*(SSB[i]<HSb[i1])) # Rec
# mu=log(HSa[i1]*(SSB[i]>=HSb[i1])+HSa[i1]*SSB[i]*(SSB[i]<HSb[i1])/HSb[i1]) # V2
    Rsym[i1,i]=mu
...
plot(SSB,exp(Rec), xlim=c(0,1.1*max(SSB)),xlab="SSB", ylab="Recruitment",ylim=c(0,12000))
...
lines(ssB,bh)
write.csv(Modset,file=paste(inPath ,"SRbayeswith R_1000models_logNorm.csv",sep=""))
